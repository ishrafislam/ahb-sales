<template>
  <div class="space-y-4">
    ...
    >
      ...
      <label class="col-span-1 text-sm">Unit
        <input
          v-model="form.unit"
          class="mt-1 w-full border rounded px-2 py-1"
        >
      </label>
      <label class="col-span-1 text-sm">Price
        <input
      ...
      <label class="col-span-1 text-sm">Stock
        <input
...
type Product = { id: number; nameBn: string; unit: string; price: number; stock: number; active: boolean }

const products = ref<Product[]>([])
const error = ref('')

const form = ref<{ id: number | null; nameBn: string; unit: string; price: number; stock: number }>({
  id: null,
  nameBn: '',
  unit: 'unit',
  price: 0,
