...
This document describes all IPC (Inter-Process Communication) channels between
the Electron main process and renderer process in the AHB Sales application.
...
**Description:** Creates a new encrypted `.ahbs` file with empty data structure.
...
**Parameters:** None  
**Returns:** `Promise<void>`  
**Description:** Opens a file picker dialog to select and decrypt an existing
`.ahbs` file.
...
**Parameters:** None  
**Returns:** `Promise<void>`  
**Description:** Saves the current document to its existing path. Does nothing
if no file is open.
...
**Parameters:** None  
**Returns:** `Promise<void>`  
**Description:** Saves the current document to a new location chosen via file
picker dialog.
...
**Description:** Posts a new sales invoice. Updates product stock and customer
account receivable.
...
**Returns:** `Promise<Invoice[]>`  
**Description:** Lists all invoices for a specific customer, sorted by date
(newest first).
...
**Description:** Lists all sales lines for a specific product across all
invoices.
...
**Description:** Generates customer-wise money transaction report for date
range.
...
**Description:** Generates day-wise money transaction report grouped by date and
customer.
...
**Description:** Manually triggers update check. Returns true if check was
initiated.
...
Events are one-way notifications from main process to renderer. They use the
`on` pattern with cleanup function.
...
- **File operations**: User cancels dialog, permission denied, encryption
  failure
...
6. **Cleanup**: Always call cleanup functions returned by event listeners to
